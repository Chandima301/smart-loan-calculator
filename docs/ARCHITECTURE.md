...
2. `LoanLandingPage` (`src/components/landing/LoanLandingPage.tsx`) renders hero → `Breadcrumb` → `LoanCalculatorShell` → `InlineRelatedCalculators` → optional `extraSection` → tab cards → `GuideSection` → `RelatedCalculators` → FAQ, and emits FAQ/Article/SoftwareApplication JSON-LD. The optional `extraSection` prop injects a page-specific SSR'd widget below the calculator (e.g. the Direct Subsidized/Unsubsidized breakdown on `/student-loan-calculator`).
