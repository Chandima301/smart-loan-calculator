...
## 2026-06-15
**Signal:** Traffic still depressed — homepage `/` 74 views (-89%), 49 sessions (47 Direct + 1 Organic Search + 1 Referral). Two firsts this run: (1) our *first-ever GA4 key event* (1, on `/`), thanks to the `analytics.ts` event wiring shipped in `403baf2`; (2) our *first Organic Search session* (previously 100% Direct). `/student-loan-calculator` stays the engaging page (4 views, +300%, 512s avg session) and remains the biggest SEO pool (569 imp / 0 clicks / pos 79.6 — SLA-001 shipped but ranking hasn't followed yet). The new `/guides` content hub is already indexing and earning: `pslf-vs-income-driven-repayment` 4 imp / 1 click / pos 41 — early validation of the IDR/discretionary-income direction (SLA-005). Largest *open* uncaptured pool: `/student-loan-payoff-calculator` 359 imp / 0 clicks / pos 72.
**Shipped since last run:** `403baf2` feat(seo) — a real app feature: biweekly extra-payment support + SSR'd "26 Biweekly = 13 Monthly" explainer (**SLA-003**), refinance "realistic break-even point" title/FAQ (**SLA-004**), GA4 `trackEvent()` key-event wiring, and the additive `/guides` content hub (3 comparison articles + hub/SSG routes + cross-links). (`51f2af4` was the 2026-06-10 insights log.)
**Prior suggestions status:** SLA-003 → **shipped** (`403baf2`). SLA-004 → **shipped** (`403baf2`). SLA-002, SLA-005, SLA-006 carried over (demand persists). New: SLA-007 (FFELP).
**Shortlist:**
- SLA-005 [M] Discretionary-income / IDR repayment plans for student-loan tooling — `/student-loan-payoff-calculator` 359 imp / 0 clicks / pos 72 (largest open pool); "discretionary income calculator" pos 61, "10 year standard repayment plan calculator" 2 imp pos 72.5, "consolidation of student loans calculator" 2 imp pos 95, "debt payoff calculator student loan" 3 imp pos 64. New `pslf-vs-idr` guide at pos 41 w/ a click validates the demand. *(carried)*
- SLA-002 [M] EMI mode/terminology on car-loan-calculator — page 75 imp / 0 clicks / pos 67.8; "automobile loan emi calculator" 27 imp pos 94.1 + 20+ "car emi" variants ("calculate auto loan emi" 4 imp pos 80.3). *(carried)*
- SLA-007 [S] FFELP student-loan content/explainer — "ffelp student loans" pos 7.5, 1 click / 2 imp (50% CTR) — the only true striking-distance query this run, and a content gap the app doesn't yet address (FFELP consolidation/PSLF-eligibility angle). *(new)*
- SLA-006 [M] Improve personal-loan-calculator (EMI + intent match) — page 34 imp / 0 clicks / pos 94.9 (worst-ranked page w/ demand); "calculation of personal loan" pos 98. *(carried)*
