...
## 2026-06-16
**Signal:** Traffic still depressed & flat — homepage `/` 71 views (-85%), ~47 sessions (45 Direct + 1 Organic + 1 Referral), down from ~61 last run. **Milestone:** the first on-site Key Event ever recorded (keyEvents=1 on `/` / Direct) — the GA4 `trackEvent` wiring shipped in `403baf2` is now live and capturing engagement (was 0 everywhere on every prior run). `/student-loan-calculator` (SLA-001 page) keeps its quality signal: 4 views (+300%) at a 512s avg session, though GSC still lags (576 imp / 0 clicks / pos 79.4 — SEO follows on a delay). Biggest open opportunity is now `/student-loan-payoff-calculator` — 380 imp / 0 clicks / pos 72.1 (+27% imp), the largest unmet pool we can act on (student-loan-calculator is bigger at 576 imp but its feature already shipped). `/pslf-calculator` demand keeps climbing (41 imp / pos 56, +24%) and the new pslf-vs-IDR guide is already converting (1 click / 25% CTR) — promoted to backlog this run (SLA-007).
**Shipped since last run:** `403baf2` feat(seo) — biweekly real extra-payment support + SSR'd "26 Biweekly = 13 Monthly Payments" H2 (**SLA-003**), refinance "realistic break-even point" title/answer/FAQ (**SLA-004**), GA4 `trackEvent()` Key Events, and an additive `/guides` content hub (3 comparison articles + hub/SSG routes + cross-links). Plus `51f2af4` (insights daily log 2026-06-10).
**Prior suggestions status:** SLA-001 already shipped (PR #1). **SLA-003 → shipped** (`403baf2`). **SLA-004 → shipped** (`403baf2`). SLA-002, SLA-005, SLA-006 carried over (demand persists on each). New: **SLA-007** (PSLF).
**Shortlist:**
- SLA-005 [M] Discretionary-income / IDR repayment plans for student-loan tooling — `/student-loan-payoff-calculator` 380 imp / 0 clicks / pos 72.1 (largest open pool, +27% imp); "calculate student loan payoff" 5 imp pos 66.6, "debt payoff calculator student loan" 3 imp pos 64, "discretionary income calculator" pos 61, "10 year standard repayment plan calculator" 2 imp pos 72.5. *(carried)*
- SLA-002 [M] EMI mode/terminology on car-loan-calculator — page 74 imp / 0 clicks / pos 67.6; "automobile loan emi calculator" 27 imp pos 94.1 + 15+ "car/auto emi" variants. *(carried)*
- SLA-007 [M] PSLF qualification/forgiveness clarity on pslf-calculator — page 41 imp / 0 clicks / pos 56 (best-ranked high-imp page w/ unmet demand, +24% imp); "does paye qualify for pslf" pos 67; new pslf-vs-IDR guide already converting (1 click / 4 imp / 25% CTR / pos 41). *(new)*
- SLA-006 [M] Improve personal-loan-calculator (EMI + intent match) — page 34 imp / 0 clicks / pos 95.1 (worst-ranked page w/ demand); "calculation of personal loan" pos 98. *(carried)*
