...
## 2026-06-14
**Signal:** Traffic still depressed & flat — homepage `/` 81 views (-88%), ~55 sessions (53 Direct + 1 Organic + 1 Referral). But a milestone: the **first-ever Key Event** is now recorded (1, homepage/desktop) — the GA4 events wired in `403baf2` (calculator_engaged, pdf_download, share_link_copy, etc.) are live and being counted, so on-site engagement is finally measurable instead of a flat 0 everywhere. `/student-loan-calculator` holds its +300% / 512s avg-session engagement on the SLA-001 page. Biggest open opportunity: `/student-loan-payoff-calculator` 334 imp / 0 clicks / pos 71.5 — the largest uncaptured search pool (up from 300) → SLA-005 (IDR / discretionary-income). Only striking-distance query is "ffelp student loans" pos 7.5, which already converts (1 click / 2 imp), so nothing to do there.
**Shipped since last run:** `403baf2` feat(seo) — Phase A striking-distance fixes (biweekly extra-payment support + "26 Biweekly = 13 Monthly" SSR H2 → **SLA-003**; refinance "realistic break-even point" title/answer/FAQ → **SLA-004**) and GA4 trackEvent instrumentation; Phase B additive `/guides` content hub (3 new comparison articles + hub/SSG routes + RelatedReading cross-links). Plus `51f2af4` (insights log 2026-06-10).
**Prior suggestions status:** SLA-003 → **shipped** (`403baf2`). SLA-004 → **shipped** (`403baf2`). SLA-002, SLA-005, SLA-006 carried over (demand persists). New: SLA-007 (PSLF) promoted off the watch list — its `/guides/pslf-vs-income-driven-repayment` article shipped this run and is already converting (1 click / 4 imp / pos 41 / 100% engagement), and the calculator page is the best-positioned open pool.
**Shortlist:**
- SLA-005 [M] Discretionary-income / IDR repayment plans for student-loan tooling — `/student-loan-payoff-calculator` 334 imp / 0 clicks / pos 71.5 (largest open pool, up from 300); "discretionary income calculator" pos 61, "10 year standard repayment plan calculator" 2 imp pos 72.5, "debt payoff calculator student loan" 3 imp pos 64, "consolidation of student loans calculator" 2 imp pos 95. *(carried)*
- SLA-002 [M] EMI mode/terminology on car-loan-calculator — page 76 imp / 0 clicks / pos 67.9; "automobile loan emi calculator" 27 imp pos 94.1 (up from 23) + 20+ "car emi" variants. *(carried)*
- SLA-007 [M] PSLF / IDR plan-qualification feature on pslf-calculator — `/pslf-calculator` 34 imp / 0 clicks / pos 56.5 (best-positioned open pool); "does paye qualify for pslf" pos 67; new `/guides/pslf-vs-income-driven-repayment` article already converting (1 click / 4 imp / pos 41 / 100% engagement). *(new)*
- SLA-006 [M] Improve personal-loan-calculator (EMI + intent match) — page 35 imp / 0 clicks / pos 94.7 (worst-ranked page w/ demand); "100000 emi for personal loan" pos 81, "calculation of personal loan" pos 98. *(carried)*
